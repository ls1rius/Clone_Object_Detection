...
from CFG import CFG
import argparse
from models.detection import retinanet_resnet_fpn
...
def train(local_rank):
    device = CFG.DEVICE
    model = retinanet_resnet_fpn(num_classes=len(CLONE_CLASSES), pretrained_backbone=True, trainable_backbone_layers=5, backbone_name='resnet18')
...
def test(local_rank):
    device = CFG.DEVICE
    model = retinanet_resnet_fpn(num_classes=len(CLONE_CLASSES), pretrained_backbone=True, trainable_backbone_layers=5, backbone_name='resnet18')
    checkpoint_dict = torch.load('./model_best_ap95.pth', map_location='cpu')
