...
from ..mmdet_module import NonLocal2D, BasicResBlock, SEBottleneck
...
#             representation_size = 1024
            box_predictor = DoubleHeadRCNNPredictor(
                representation_size,
                num_classes)
...
class DoubleHeadRCNNPredictor(nn.Module):
    """
    DoubleHeadRCNNPredictor classification + bounding box regression layers
    for Fast R-CNN.
    ...
    def __init__(self, in_channels, num_classes):
        super(DoubleHeadRCNNPredictor, self).__init__()
        self.conv_out_channels = 1024
        self.fc_out_channels = 1024
        self.num_convs = 4
        self.num_fcs = 2
        self.roi_feat_area = 7 * 7
#         self.nonlocal = NonLocal2D()
        
        self.res_block = BasicResBlock(in_channels, self.conv_out_channels)
        ...
        for _ in range(self.num_fcs):
            fc_in_channels = (in_channels * self.roi_feat_area if _ == 0 else self.fc_out_channels)
            self.fc_branch.append(nn.Linear(fc_in_channels, self.fc_out_channels))
        
        self.fc_reg = nn.Linear(self.conv_out_channels, num_classes * 4)
    ...
    def forward(self, x):
        ...
        bbox_deltas = self.fc_reg(x_conv)

        # fc head
        x_fc = x.flatten(start_dim=1)
        x_fc = x_fc.view(x_fc.size(0), -1)
        for fc in self.fc_branch:
            x_fc = self.relu(fc(x_fc))
        scores = self.fc_cls(x_fc)
...
def fasterrcnn_resnet_fpn(pretrained=False, progress=True, backbone_name='resnet50',
...
#     if pretrained:
#         state_dict = load_state_dict_from_url(model_urls['fasterrcnn_' + backbone_name + '_fpn_coco'],
#                                               progress=progress)
