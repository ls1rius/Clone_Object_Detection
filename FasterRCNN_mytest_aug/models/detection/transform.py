...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def __init__(self, min_size, max_size, image_mean, image_std, size_divisible=32, fixed_size=None):
        ...
        self.detection_transforms = self.get_detection_transforms()
    ...
    def forward(self,
        ...
        for i in range(len(images)):
            ...
            # L10 ADD
            if training:
                image = self.do_detection_transforms(image, target_index['boxes'].data.cpu().numpy().astype(int))
    ...
    def get_detection_transforms(self):
#         transforms_list = []
        transforms_list = [
    ...
    def do_detection_transforms(self, image, boxes):
        # 将object拿出来作变换再放回去
        for box in boxes:
            obj_img = image[:, box[1]:box[3], box[0]:box[2]]
            image[:, box[1]:box[3], box[0]:box[2]] = self.detection_transforms(obj_img)
        return image
